...
<div class="wrapper bg-white mt-sm-5">
    ...
    <h4 class="pb-4 border-bottom">Edit contact</h4>
    ...
    <div class="py-2">
        ...
        <div class="py-3 pb-4">
            ...
            <router-link v-if="contact._id" :to="{ name: 'ContactDetails', params: { id: contact._id } }" class="btn btn-outline-secondary mt-1">Cancel</router-link>
        </div>
...
export default {
  ...
  methods: {
	...
	update() {
        ApiHelper.updateContact({ token: this.$store.state.token, id: this.contact._id, contact: this.contact })
		.then( response => {
			if(response.data.success)
				this.$router.push({ name: 'ContactDetails', params: { id: this.contact._id } })
			else
				throw response
		})
	}
